...
case $1 in
    deploy)
        ...
        #I ADDED THIS
        SERVICES="valkey scraping_nigeria db vcca_air_service farmer_scheduler farmer_web impact_dashboard_web web gateway celery impact_dashboard_scheduler"


        # This will probably only be needed by dev. I MADE CHANGES BELOW TOO
        $DOCKER_COMPOSE_CMD build $SERVICES
        # Pull the most recent images based on the given configuration
        $DOCKER_COMPOSE_CMD pull $SERVICES
        ...
        # We should be ready to now rollout the rest of the dependencies. I MADE A CHANGES HERE TO PREVENT COMSOL AND MLINDIA FROM STARTING
        exec $DOCKER_COMPOSE_CMD up -d $SERVICES;
