...
class ImpactSlicer(BaseSlicer):
    def slice_table(
        ...
        farmer_filter = "AND farmer_id = %s" if farmer_id else ""
        query_1 = f"""
                SELECT
                    {group_by_field},
        ...
        if mode == "company":
            ...
            if farmer_id:
                params = params + (farmer_id,)
            return (
                pd.read_sql(query_1, self.conn, params=params),
                aggregated_df,
            )
        else:
            data = {}
