...
survey_prices_checkout as (
    SELECT uf.id as farmer_id,
        ...
        AVG(oms.price) AS selling_price, -- price is entered per kg, no division needed
...
baseline_survey as (
    SELECT
        ...
        AVG(ufsc.average_price) AS baseline_prices, -- price is entered per kg, no division needed
        MIN(ufsc.quantity_sold) AS quantity_sold,
        MIN(ufsc.quantity_self_consumed) AS quantity_self_consumed,
        MAX(ufsc.quantity_total) AS quantity_total,
        MIN(ufsc.quantity_below_market_price) AS quantity_lost,
...
checkout_summaries AS (
    SELECT
        oc.id AS checkout_id,
        sc.cooling_unit_id,
        oc2.owned_by_user_id AS user_id,
        sp.crop_id,
        SUM(scpc.weight_in_kg) AS total_weight,
        SUM(sc.initial_weight) AS checkin_weight, -- actual kg that entered the cold room
        MAX(oms.loss) AS checkout_loss,
        MAX(oms.price) AS selling_price,
        MAX(oms.currency) AS checkout_currency,
        MAX(oms.date_filled_in) AS date_filled_in,
        MAX(oms.kg_in_unit) AS kg_in_unit
    ...
    LEFT JOIN operation_marketsurvey oms ON oms.checkout_id = oc.id
    JOIN storage_produce sp ON sp.id = sc.produce_id
    JOIN operation_checkin oc2 ON sp.checkin_id = oc2.id
    JOIN operation_movement om_co ON om_co.id = oc.movement_id
    WHERE
        om_co.date >= %(datetime_start)s
        AND om_co.date < %(datetime_end)s
    GROUP BY oc.id, sc.cooling_unit_id, oc2.owned_by_user_id, sp.crop_id
)
SELECT
    %(date_end)s as report_date,
    cs.cooling_unit_id AS cooling_unit_id,
    MAX(bs.cooling_unit_name) as unit_name,
    MAX(bs.company_id) AS company_id,
    uf.id AS farmer_id,
    cs.crop_id AS crop_id,
    uu.first_name AS first_name,
    uu.last_name AS last_name,
    scp.name AS crop_name,
    COALESCE(MAX(cs.checkout_currency), MAX(bs.baseline_currency)) AS currency,
    ...
    (MAX(bs.quantity_lost) / NULLIF(MAX(bs.quantity_total), 0)) * 100 AS baseline_perc_loss_month,
    MAX(bs.quantity_sold) AS baseline_kg_sold_month,
    SUM(cs.checkin_weight) * MAX(bs.baseline_prices) AS baseline_farmer_revenue_month, -- actual kg checked in × pre-coldroom price
    MAX(COALESCE(casp.selling_price, 0)) AS monthly_kg_selling_price,
    SUM(cs.total_weight) AS monthly_kg_checkin,
    SUM(COALESCE(cs.checkout_loss, 0)) AS monthly_kg_loss,
    MAX(COALESCE(casp.selling_price, 0)) * (SUM(cs.total_weight) - SUM(COALESCE(cs.checkout_loss, 0))) AS monthly_farmer_revenue,
    MAX(COALESCE(casp.selling_price, 0)) - MAX(bs.baseline_prices) AS monthly_kg_selling_price_evolution,
    (((MAX(COALESCE(casp.selling_price, 0)))- MAX(bs.baseline_prices)) / NULLIF(MAX(bs.baseline_prices), 0)) * 100 AS monthly_perc_unit_selling_price_evolution,
    ( (MAX(COALESCE(casp.selling_price, 0)) * (SUM(cs.total_weight) - SUM(COALESCE(cs.checkout_loss, 0))))  - ((SUM(cs.total_weight) - SUM(COALESCE(cs.checkout_loss, 0))) * MAX(bs.baseline_prices))) AS monthly_farmer_revenue_evolution,
    (( (MAX(COALESCE(casp.selling_price, 0)) * (SUM(cs.total_weight) - SUM(COALESCE(cs.checkout_loss, 0))))  - ((SUM(cs.total_weight) - SUM(COALESCE(cs.checkout_loss, 0))) * MAX(bs.baseline_prices)))  / NULLIF(((SUM(cs.total_weight) - SUM(COALESCE(cs.checkout_loss, 0))) * MAX(bs.baseline_prices)), 0)) * 100 AS monthly_perc_farmer_revenue_evolution,
    (SUM(COALESCE(cs.checkout_loss, 0)) / NULLIF(SUM(cs.total_weight), 0)) * 100 AS monthly_perc_loss,
    ((SUM(COALESCE(cs.checkout_loss, 0)) / NULLIF(SUM(cs.total_weight), 0)) - (MAX(bs.quantity_lost) / NULLIF(MAX(bs.quantity_total), 0)))  * 100 AS monthly_perc_foodloss_diff,
    CASE
        WHEN MAX(bs.quantity_lost) = 0
            THEN (SUM(COALESCE(cs.checkout_loss, 0)) / NULLIF(SUM(cs.total_weight), 0)) * 100
        ELSE (
            ((SUM(COALESCE(cs.checkout_loss, 0)) / NULLIF(SUM(cs.total_weight), 0)) - (MAX(bs.quantity_lost) / NULLIF(MAX(bs.quantity_total), 0))) /
            NULLIF((MAX(bs.quantity_lost) / NULLIF(MAX(bs.quantity_total), 0)), 0)
        ) * 100
    END AS monthly_perc_foodloss_evolution,
    MAX(cs.date_filled_in) AS latest_survey_date,
...
FROM checkout_summaries cs
join user_user uu on (cs.user_id = uu.id)
join user_farmer uf on (uf.user_id = uu.id)
JOIN storage_crop scp on scp.id = cs.crop_id
JOIN baseline_survey bs on cs.cooling_unit_id = bs.cooling_unit_id
    AND uf.id = bs.farmer_id
    AND scp.name = bs.crop_name
LEFT JOIN baseline_completed_surveys bcs on bcs.id = cs.cooling_unit_id
LEFT JOIN baseline_possible_surveys bps on bps.cooling_unit_id = cs.cooling_unit_id
LEFT JOIN total_post_checkout_survey_count tpcs on tpcs.cooling_unit_id = cs.cooling_unit_id
LEFT JOIN cooling_unit_aggregated_survey_prices casp on casp.cooling_unit_id = cs.cooling_unit_id AND casp.farmer_id = uf.id AND casp.crop_name = scp.name
WHERE
    (
       bs.baseline_currency = cs.checkout_currency
       OR (cs.checkout_currency IS NULL AND bs.baseline_currency IS NOT NULL)
...
GROUP BY cs.cooling_unit_id, uf.id, uu.first_name, uu.last_name, scp.name, cs.crop_id;
