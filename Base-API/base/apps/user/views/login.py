...
class LoginViewSet(TokenObtainPairView):
    ...
    def post(self, request, *args, **kwargs):
        logger.info(f"Login request received - user_type: {request.data.get(USER_TYPE_PARAM, 'MISSING')}, username: {request.data.get('username', 'MISSING')}")
        try:
        ...
        try:
            ...
            logger.info(f"Selected serializer: {serializer_class.__name__ if serializer_class else 'None'}")
            ...
            logger.error(f"Login processing error: {type(e).__name__}: {str(e)}")
            raise
    ...
    def get_serializer_class(self):
        ...
        if USER_TYPE_PARAM in self.request.data:
            ...
            logger.warning(f"user_type parameter '{USER_TYPE_PARAM}' not found in request data. Keys: {list(self.request.data.keys())}")
        return None
