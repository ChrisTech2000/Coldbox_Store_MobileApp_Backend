...
class SellerListedCratesViewSet(ViewSet):
    ...
    def create(self, request):
        ...
        total_weight_left = input_serializer.validated_data.get('total_listed_weight')
        picture_file = input_serializer.validated_data.get('picture')
        listings = []
        ...
        for crate in crates:
            ...
            # Assign cascaded limit if total_weight_left is specified:
            if total_weight_left is not None:
                allowable = min(total_weight_left, crate.weight)
                listing.listed_weight_limit = allowable
                total_weight_left = max(0, total_weight_left - allowable)
            else:
                listing.listed_weight_limit = None
            
            if picture_file:
                listing.picture = picture_file
