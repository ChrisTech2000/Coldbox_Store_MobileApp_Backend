...
class BuyerCartItemsViewSet(ViewSet):
    ...
    def create(self, request):
        ...
        # Identify the group of crates belonging to the same check-in movement and crop
        group_crates = MarketListedCrate.objects.filter(
            crate__produce__checkin__movement__code=crate.produce.checkin.movement.code,
            crate__produce__crop_id=crate.produce.crop_id,
            delisted_at__isnull=True
        ).order_by('created_at')

        # Get existing order items for this group
        existing_items = OrderCrateItem.objects.filter(
            order=cart, market_listed_crate__in=group_crates
        )

        current_group_weight = sum(item.ordered_produce_weight for item in existing_items)

        if update_strategy == "replace":
            new_total_group_weight = req_ordered_produce_weight
        elif update_strategy == "increase":
            new_total_group_weight = current_group_weight + req_ordered_produce_weight
        elif update_strategy == "decrease":
            new_total_group_weight = current_group_weight - req_ordered_produce_weight
        else:
            new_total_group_weight = req_ordered_produce_weight
            
        new_total_group_weight = max(0, new_total_group_weight)

        # Clear existing allocations so we can safely waterfall the new total
        if existing_items.exists():
            existing_items.delete()

        # Recompute available weight for all grouped crates now that items are wiped
        for gc in group_crates:
            gc.compute(save=True)

        # Waterfall the requested weight across available crates
        remaining_to_allocate = new_total_group_weight
        created_item = False
        ...
        for gc in group_crates:
            ...
            allowable = max(0, min(gc.cmp_available_weight_in_kg, remaining_to_allocate))
            if allowable > 0:
                OrderCrateItem.objects.create(
                    order=cart,
                    market_listed_crate=gc,
                    ordered_produce_weight=allowable
                )
                remaining_to_allocate -= allowable
                created_item = True
