...
class MarketListedCrate(models.Model):
    ...
    listed_weight_limit = models.FloatField(_("listed_weight_limit"), null=True, blank=True)
    picture = models.FileField(_("picture"), upload_to="marketplace/listed_crates/", null=True, blank=True)
    ...
    def compute(self, save=True):
        ...
        effective_crate_weight = self.listed_weight_limit if self.listed_weight_limit is not None else crate_weight
        effective_crate_weight = min(effective_crate_weight, crate_weight)
        
        self.cmp_available_weight_in_kg = max(0, effective_crate_weight - self.cmp_weight_locked_in_payment_pending_orders_in_kg)
