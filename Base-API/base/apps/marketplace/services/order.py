...
def process_order_completion(order, payment_through=None, payment_gateway=None, payment_method=None):
    ...
    for market_listed_crate, ordered_entire_crate in listings_to_be_computed:
        if market_listed_crate.delisted_at is None:
            ...
            # Refresh the crate from DB before computing, because Django caches FK
            # relations in memory. create_partial_checkout() saved the reduced weight
            # to the DB, but market_listed_crate.crate still holds the old value.
            market_listed_crate.crate.refresh_from_db()
