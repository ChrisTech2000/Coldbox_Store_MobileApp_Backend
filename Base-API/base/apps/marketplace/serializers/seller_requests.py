...
class SellerListingUpdateRequestSerializer(serializers.Serializer):
    ...
    total_listed_weight = serializers.FloatField(
        required=False,
        help_text="Optional total exact weight restriction to cascade across the selected crates",
        min_value=0.0
    )
    picture = serializers.FileField(
        required=False,
        allow_null=True,
        help_text="Optional picture for the marketplace listing"
    )

    class Meta:
        fields = ['produce_price_per_kg', 'crate_ids', 'total_listed_weight', 'picture']
