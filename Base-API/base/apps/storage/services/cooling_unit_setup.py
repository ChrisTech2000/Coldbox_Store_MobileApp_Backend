...
def update_cooling_unit_from_payload(payload, cooling_unit_id):
        ...
        if crop_id not in current_crop_ids:
            ...
            try:
                ...
                if old_pricing is None:
                    # Create new pricing if it didn't exist
                    new_pricing = Pricing.objects.create(
                        pricing_type=cu_data["pricing_type"],
                        fixed_rate=cu_data["fixed_rate"],
                        daily_rate=cu_data["daily_rate"],
                    )
                    cuc.pricing = new_pricing
                    cuc.save()
                    should_recalculate_crates = True
                else:
                    ...
                    Pricing.objects.filter(id=old_pricing.id).update(
                        pricing_type=cu_data["pricing_type"],
                        fixed_rate=cu_data["fixed_rate"],
                        daily_rate=cu_data["daily_rate"],
                    )
    ...
    pricing_id = payload.get("pricing_id")
    if pricing_id:
        try:
            default_pricing = Pricing.objects.get(id=pricing_id)
