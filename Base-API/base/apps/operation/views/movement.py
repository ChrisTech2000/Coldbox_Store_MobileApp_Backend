    ...
    def get_checkouts_usage(self, request, *args, **kwargs):
        ...
        qs = (
            self.model.objects.filter(
                ...
                | Q(checkouts__partial_checkouts__crate__cooling_unit__in=cooling_units)
            ).distinct()
        )
        # Use simple serializer instead of optimized_init to avoid
        # the DISTINCT + complex JOINs dropping checkout movements
        serializer = MovementSerializer(qs, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
